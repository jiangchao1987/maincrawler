...
public class AppDao {
	...
	public static void updateAppPrice(int applicationId, String priceTrend, float lastPrice, float currentPrice) {
        try {
            ...
            PreparedStatement ps = connection.prepareStatement("update tb_application set last_price = ?, current_price = ?, price_trend = ?, updated_at = ?, expire_datetime = ?, display_order = ?, display_order2 = ? where application_id = ?");
