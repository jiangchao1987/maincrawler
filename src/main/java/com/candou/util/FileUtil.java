...
public class FileUtil {
    ...
    public static void remoteFile(String url, File target, boolean debug, int threadCount) {
        url = url.replace(" ", "%20"); // 将url中的空格替换成%20, 使用encoder会把空格替换成+,
                                       // 不符合要求
    ...
    public static void main(String[] args) throws InterruptedException {
        for (int index = 0; index < 10; index++) {
            // String url =
            // "http://download.shendu.com/downloads/rom/886-shendu.com-samsung-s5830-cm-7.2.0-cooper-Sip-signed.zip";
            String url = "http://wx.ijinshan.com/i/1095-0.jpg?1362246001";
            File target = new File("F:\\" + index + ".jsg");
    ...
    public static String generatePartFolderName(String url, String replaceName) {
        String foldername = url.replace(replaceName, "");
        foldername = foldername.substring(foldername.indexOf("/") + 1, foldername.lastIndexOf("/"));
        return foldername;
    }

    public static String generateFileName(String expansion) {
        return getUUID().concat(".").concat(expansion);
    }

    public static String getUUID() {
        String s = UUID.randomUUID().toString();
        // 去掉“-”符号
        s = s.substring(0, 8).concat(s.substring(9, 13)).concat(s.substring(14, 18)).concat(s.substring(19, 23))
            .concat(s.substring(24));
        s = s.concat(String.valueOf(System.currentTimeMillis()));
        return s;
    }
}
